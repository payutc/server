 ...
 */
abstract class BaseTSaleSal extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    const PEER = 'TSaleSalPeer';
    ...
    /**
     * Initializes internal state of BaseTSaleSal object.
     * @see        applyDefaults()
     */
     ...
     * @return TSaleSal The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->sal_id !== $v) {
            $this->sal_id = $v;
            $this->modifiedColumns[] = TSaleSalPeer::SAL_ID;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->sal_name !== $v) {
            $this->sal_name = $v;
            $this->modifiedColumns[] = TSaleSalPeer::SAL_NAME;
        }
    ...
    public function setPerId($v)
    {
        ...
        if ($this->per_id !== $v) {
            $this->per_id = $v;
            $this->modifiedColumns[] = TSaleSalPeer::PER_ID;
        }
    ...
    public function setObjId($v)
    {
        ...
        if ($this->obj_id !== $v) {
            $this->obj_id = $v;
            $this->modifiedColumns[] = TSaleSalPeer::OBJ_ID;
        }
    ...
    public function setRemoved($v)
    {
        ...
        if ($this->sal_removed !== $v) {
            $this->sal_removed = $v;
            $this->modifiedColumns[] = TSaleSalPeer::SAL_REMOVED;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 5; // 5 = TSaleSalPeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating TSaleSal object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        $stmt = TSaleSalPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = TSaleSalQuery::create()
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TSaleSalPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(TSaleSalPeer::SAL_ID)) {
            $modifiedColumns[':p' . $index++]  = '`SAL_ID`';
        }
        if ($this->isColumnModified(TSaleSalPeer::SAL_NAME)) {
            $modifiedColumns[':p' . $index++]  = '`SAL_NAME`';
        }
        if ($this->isColumnModified(TSaleSalPeer::PER_ID)) {
            $modifiedColumns[':p' . $index++]  = '`PER_ID`';
        }
        if ($this->isColumnModified(TSaleSalPeer::OBJ_ID)) {
            $modifiedColumns[':p' . $index++]  = '`OBJ_ID`';
        }
        if ($this->isColumnModified(TSaleSalPeer::SAL_REMOVED)) {
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = TSaleSalPeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = TSaleSalPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['TSaleSal'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['TSaleSal'][$this->getPrimaryKey()] = true;
        $keys = TSaleSalPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TSaleSalPeer::SAL_ID)) $criteria->add(TSaleSalPeer::SAL_ID, $this->sal_id);
        if ($this->isColumnModified(TSaleSalPeer::SAL_NAME)) $criteria->add(TSaleSalPeer::SAL_NAME, $this->sal_name);
        if ($this->isColumnModified(TSaleSalPeer::PER_ID)) $criteria->add(TSaleSalPeer::PER_ID, $this->per_id);
        if ($this->isColumnModified(TSaleSalPeer::OBJ_ID)) $criteria->add(TSaleSalPeer::OBJ_ID, $this->obj_id);
        if ($this->isColumnModified(TSaleSalPeer::SAL_REMOVED)) $criteria->add(TSaleSalPeer::SAL_REMOVED, $this->sal_removed);
    ...
    {
        $criteria = new Criteria(TSaleSalPeer::DATABASE_NAME);
        $criteria->add(TSaleSalPeer::SAL_ID, $this->sal_id);
     ...
     * objects.
     *
     * @param object $copyObj An object of TSaleSal (or compatible) type.
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return TSaleSal Clone of current object.
     * @throws PropelException
     */
    ...
    public function getTObjectObj(PropelPDO $con = null)
    {
        if ($this->aTObjectObj === null && ($this->obj_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aTObjectObj->addTSaleSals($this);
             */
        }
    ...
    public function getTPeriodPer(PropelPDO $con = null)
    {
        if ($this->aTPeriodPer === null && ($this->per_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aTPeriodPer->addTSaleSals($this);
             */
        }
    ...
    {
        return (string) $this->exportTo(TSaleSalPeer::DEFAULT_STRING_FORMAT);
    }
