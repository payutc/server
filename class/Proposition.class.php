        ...
        if ($ParentObject != 0 AND $ParentObject->getType() == 'category') {
            ...
            if ($DetailsPromo != 0) {
                ...
                foreach ($DetailsPromo as $key => $value) {
                    if ($value == 'product') $condition.= "obj.obj_id = '" . $key . "' OR ";
                    else if ($value == 'category') $condition.= "oli.obj_id_parent = '" . $key . "' OR ";
                }
...
per4.per_date_end >= NOW() AND";
        if ($userid != 0) $req.= " jug.usr_id = @BUYER_ID AND ";
        $req.= "
...
;";
        ...
        if ($this->db->affectedRows() >= 1) {
            while ($don = $this->db->fetchArray($res)) {
                if ($ParentObject != 0 AND $ParentObject->getType() == 'promotion') $temp_obj = new ObjectWithPrice($don['obj_id'], $don['obj_name'], $don['obj_type'], $don['obj_stock'], $don['fun_id'], $don['img_id'], 0, 0);
                else $temp_obj = new ObjectWithPrice($don['obj_id'], $don['obj_name'], $don['obj_type'], $don['obj_stock'], $don['fun_id'], $don['img_id'], 0, $don['credit']);
