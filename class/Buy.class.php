...
class Buy extends WsdlBase {
    ...
    public function getTypeRecharge($type = '') {
        ...
        if ($type == '') $res = $this->db->query("SELECT rty_id, rty_name FROM t_recharge_type_rty WHERE rty_removed = '0' ORDER BY rty_id ASC;");
        else $res = $this->db->query("SELECT rty_id, rty_name FROM t_recharge_type_rty WHERE rty_removed = '0' AND rty_type = '%s' ORDER BY rty_id ASC;", Array($type));
    ...
    public function getPropositions(&$Seller) {
        if (isset($this->SelectObject) AND ($this->SelectObject->getType() == 'category')) $Proposition = new Proposition($this->Seller, $this->Buyer, $this->Point, $this->SelectObject);
        else if (isset($this->SelectObject) AND ($this->SelectObject->getType() == 'promotion')) $Proposition = new Proposition($this->Seller, $this->Buyer, $this->Point, $this->SelectObject, $this->promo_step);
        else $Proposition = new Proposition($this->Seller, $this->Buyer, $this->Point);
    ...
    protected function reload($amount, $rty_id, $trace, $id_operator) {
        ...
        if ($this->db->numRows($this->db->query("SELECT rty_id FROM t_recharge_type_rty WHERE rty_id = '%u' AND rty_removed = '0';", Array($rty_id))) != 1) return 452;
        //test si il n'a pas dépassé le seuil max
        $Buyer_credit = $this->Buyer->getCredit(); //évite de faire 2 fois de suite la même requête
        if ($Buyer_credit >= $this->MAX_CREDIT) return 450;
        if (($Buyer_credit + $amount) > $this->MAX_CREDIT) return 451;
    ...
    protected function endTransaction() {
        if ((isset($this->SelectObject)) OR ($this->promo_step != 0)) return 414;
        unset($this->Buyer);
        unset($this->SelectObject);
        $this->promo_step = 0;
        return 1;
