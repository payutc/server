...
class Paybox {
    ...
    public function execute($amount, $callback_url, $mobile = 0) {
        global $_CONFIG;
        $pay_id = $this->db->insertId($this->db->query("INSERT INTO  `t_paybox_pay` (
`pay_id` ,
`usr_id` ,
`pay_step` ,
`pay_amount` ,
...
NULL ,  '%u',  'W',  '%u', NOW( ) , NULL , NULL , NULL ,  '%s',  '%u', NULL
        ...
        if ($mobile == 1) {
            ...
            $pbx_url = "https://preprod-tpeweb.paybox.com/cgi/MYchoix_pagepaiement.cgi";
        }
        $PBX.= " PBX_PAYBOX=$pbx_url";
        $PBX.= " PBX_BACKUP1=$pbx_url";
        $PBX.= " PBX_BACKUP2=$pbx_url";
        // PROXY
        if (isset($_CONFIG['proxy'])) $PBX.= " PBX_PROXY=" . $_CONFIG['proxy'];;
    ...
    static public function PBXretour() {
        ...
        if ($CheckSig != 1) {
            Log::write("PAYBOX : La signature est fausse ! \n" . print_r($_GET, true), 10);
        } else {
            ...
            if ($db->affectedRows() != 1) {
                ...
                if ($amount != $paybox_row['pay_amount']) {
                    Log::write("PAYBOX : Paybox renvoit un montant de : $amount, mais le rechargement initial était de : " . $paybox_row['pay_amount'] . " \n" . print_r($_GET, true), 10);
    ...
    static private function LoadKey($keyfile, $pub = true, $pass = '') { // chargement de la clé (publique par défaut)
        ...
        if ($pub) $key = openssl_pkey_get_public($filedata); // recuperation de la cle publique
        else
        // ou recuperation de la cle privee
