...
class User {
    ...
    public function __construct($data, $meanOfLogin, $pass, $ip = 0, $noPass = 0, $checkBlocked = 1) {
        ...
        if ($noPass == 0) {
            ...
            if ($this->checkPass($pass) != 1) return $this->state;
        }
    ...
    public function getCredit() {
        ...
        if ($this->db->affectedRows() == 1) return $this->credit;
        else return 400;
    }
    /**
     * Retourne $photo
    ...
    public function blockMe() {
        $this->db->query("UPDATE ts_user_usr SET usr_fail_auth=0, usr_blocked='1' WHERE usr_id='%u'", array($this->idUser));
        if ($this->db->affectedRows() == 1) return 1;
        else return 400;
    }
    /**
    ...
    public function isInGroup($grp_id) {
        if (array_key_exists($grp_id, $this->groups)) return 1;
        else return 0;
    }
    /**
        ...
        foreach ($this->droits as $key => $value) {
            if (in_array($rig_name, $value)) {
                if (($fun_id != 0) && ($poi_id != 0)) {
                    if ($value['fun_id'] == $fun_id AND $value['poi_id'] == $poi_id) $return = 1;
                } elseif ($fun_id != 0) {
                    if ($value['fun_id'] == $fun_id) $return = 1;
                } elseif ($poi_id != 0) {
                    if ($value['poi_id'] == $poi_id) $return = 1;
                } else {
                    $return = 1;
                }
            }
    ...
    public function isAdminBuckutt() {
        if ($this->isBloqueur() == 1 || $this->isDroitAdmin() == 1 || $this->isBuckuttTrezo() == 1 || $this->isPointAdmin() == 1 || $this->isRespFundations() == 1) return 1;
        else return 456;
    }
    /**
    ...
    public function isBloqueur() {
        if ($this->hasDroit("bloqueur", 0, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isDroitAdmin() {
        if ($this->hasDroit("droit_admin", 0, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isBuckuttTrezo() {
        if ($this->hasDroit("buckutt_trezo", 0, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isPointAdmin() {
        if ($this->hasDroit("point_admin", 0, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isRespFundations() {
        if ($this->hasDroit("resp_fundations", 0, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isVenteAdmin($fun_id) {
        if ($this->hasDroit("vente_admin", $fun_id, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isGroupEditor($fun_id) {
        if ($this->hasDroit("group_editor", $fun_id, 0) == 1) return 1;
        else return 454;
    }
    /**
    ...
    public function isFundTrezo($fun_id) {
        if ($this->hasDroit("fund_trezo", $fun_id, 0) == 1) return 1;
        else return 454;
    }
    /** 
     *
    ...
    public function getLastPurchase() {
        $res = Db_buckutt::getInstance()->query("SELECT pur_id, obj_id, pur_price FROM t_purchase_pur WHERE UNIX_TIMESTAMP(pur_date) > (UNIX_TIMESTAMP(NOW()) - 900) AND usr_id_buyer = %u AND pur_removed = 0", array($this->idUser));
        $pur = array();
