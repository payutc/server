...
class ApplicationRight {
    ...
    public static function check($application_id = false, $service_name = false, $fundation_id = false) {
        ...
        if ($fundation_id) $res = $db->query($req . " AND (afu.fun_id = '%u' OR afu.fun_id IS NULL)", array($application_id, $service_name, $fundation_id));
        else $res = $db->query($req, array($application_id, $service_name));
        if ($db->affectedRows() == 0) {
            if ($fundation_id) throw new \Exception("L'application_id $application_id n'a pas les droits $service_name sur la fundation n°$fundation_id");
            else throw new \Exception("L'application_id $application_id n'a les droits $service_name sur aucune fundation");
        }
        return "ok";
    }
    /**
