...
WHERE poi.poi_id = pla.poi_id AND pla.pla_id = '%u' ORDER BY pla.pla_start;", Array($this->id));
    ...
    public function insert() {
        if ($this->time_start > $this->time_end) return array("error" => 400, "error_msg" => "L'heure de début ne peut pas être supérieur à l'heure de fin.");
        $right = new CheckRight($this->user->getId(), $this->poi_id, $this->fun_id);
        if (!$right->check("ADMIN")) return array("error" => 400, "error_msg" => "Vous n'avez pas le droit ADMIN sur cette fundation.");
        if (!$right->check("POI-FUNDATION")) return array("error" => 400, "error_msg" => "La fundation n'a pas le droit d'utiliser ce POI.");
        else {
    ...
    FROM t_plage_pla pla, t_point_poi poi
    WHERE poi.poi_id = pla.poi_id AND pla.fun_id = '%u' AND pla.poi_id = '%u' AND pla.pla_start < '%u' AND pla.pla_end > '%u' ORDER BY pla.pla_start;", Array($this->fun_id, $this->poi_id, $this->time_end, $this->time_start));
            if ($this->db->affectedRows() >= 1) {
            ...
            // INSERTION
            $this->db->query("INSERT INTO  `payutc`.`t_plage_pla` (
`pla_id` ,
`fun_id` ,
`poi_id` ,
`pla_start` ,
...
NULL ,  '%u',  '%u',  '%u', '%u',  '%s'
    ...
    public function rm() {
        ...
        $right = new CheckRight($this->user->getId(), $this->poi_id, $this->fun_id);
        if (!$right->check("ADMIN")) return array("error" => 400, "error_msg" => "Vous n'avez pas le droit ADMIN sur cette fundation.");
        $this->db->query("DELETE FROM t_plage_pla WHERE pla_id = '%u';", array($this->id));
        return array("success" => "ok");
    }
    /**
...
WHERE poi.poi_id = pla.poi_id AND pla.fun_id = '%u' ORDER BY pla.pla_start;", Array($fun_id));
        while ($don = $this->db->fetchArray($res)) {
            $plages[] = array("id" => $don['pla_id'], "poi_id" => $don['poi_id'], "poi_name" => $don['poi_name'], "start" => $don['pla_start'], "end" => $don['pla_end'], "name" => $don['pla_name']);
        }
