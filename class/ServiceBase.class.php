...
class ServiceBase {
    ...
    public function logout() {
        ...
        if ($this->app) unset($this->application);
        session_destroy();
        return "ok";
    }
    /**
    ...
    public function getStatus() {
        if ($this->application) $app = $this->application->to_array($key = 0);
        else $app = null;
        if ($this->user) $user = $this->user->getNickname();
        else $user = null;
    ...
    public function checkRight($user = true, $app = true, $fun_id = false) {
        if ($user) {
            if (!$this->user) throw new Exception("Vous devez connecter un utilisateur ! (method loginCas)");
            // Check if App_id <=> Fun_id <=> Service_name exists in ApplicationRight
            UserRight::check($user_id = $this->user->getId(), $service_name = $this->service_name, $fundation_id = $fun_id);
        }
        if ($app) {
            if (!$this->application) throw new Exception("Vous devez connecter une application ! (method loginApp)");
            // Check if App_id <=> Fun_id <=> Service_name exists in ApplicationRight
            ApplicationRight::check($application_id = $this->application->getId(), $service_name = $this->service_name, $fundation_id = $fun_id);
        }
        return "ok";
    }
    /**
    ...
    public function getFundations() {
        ...
        $fundations = array();
        foreach ($fundations_for_user as $fun_id => $fundation) if (array_key_exists($fun_id, $fundations_for_app)) $fundations[$fun_id] = $fundation;
        return $fundations;
    }
    /**
