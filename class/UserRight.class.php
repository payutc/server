...
class UserRight {
    ...
    public static function check($user_id = false, $service_name = false, $fundation_id = false) {
        ...
        if ($fundation_id) $res = $db->query($req . " AND (ufu.fun_id = '%u' OR ufu.fun_id IS NULL)", array($user_id, $service_name, $fundation_id));
        else $res = $db->query($req, array($user_id, $service_name));
        if ($db->affectedRows() == 0) {
            if ($fundation_id) throw new \Exception("Le user_id $user_id n'a pas les droits $service_name sur la fundation n°$fundation_id");
            else throw new \Exception("Le user_id $user_id n'a les droits $service_name sur aucune fundation");
        }
        return "ok";
    }
    /**
