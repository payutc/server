...
class CheckRight {
    ...
    public function check($right) {
        ...
        if (in_array($right_id, $right_user_fundation)) $res = $this->db->query("SELECT jur_id
						FROM tj_usr_rig_jur
						WHERE usr_id = '%u' AND rig_id = '%u' AND fun_id = '%u';", array($this->user_id, $right_id, $this->fun_id));
        else if (in_array($right_id, $right_poi_fundation)) $res = $this->db->query("SELECT jur_id
						FROM tj_usr_rig_jur
