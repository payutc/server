...
echo $FADMIN->setIdFundation(2);

//echo $FADMIN->getPurchasedObjects(time() - 24*3600*10, time(), 3);

//echo '<h2>Total</h2>';

//echo $FADMIN->getTotal(time() - 24*3600*10, time(), 3);

echo '<h2>Groupe</h2>';

echo $FADMIN->getAllGroups();
echo $FADMIN->editGroupName(2, 'tioototot');
echo $FADMIN->getAllGroups();
...
echo '<h2>client</h2>';
$client = new SoapClient("http://buckutt.dyndns.org/server/FADMIN.class.php?wsdl");
echo $client->login('bernardx', 1, '5546', 'ttt');

echo $client->getPurchasedObjects(time() - 24*3600*10, time(), 3);

echo '<h2>Total</h2>';

echo $client->getTotal(time() - 24*3600*10, time(), 3);
