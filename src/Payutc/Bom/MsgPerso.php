    ...
    public function setMsgPerso($msgPerso, $usrId = NULL, $funId = NULL)
    {
        ...
        if (mb_check_encoding($msgPerso, 'UTF-8')) {
            if (strlen($msgPerso) < 255){
                ...
                } else if (($usrId == NULL) && ($funId != NULL))  {
                    $db->query("INSERT INTO t_message_msg (usr_id, fun_id, msg_perso) VALUES (NULL, '%u', '%s') ON DUPLICATE KEY UPDATE msg_perso='%s'", Array($funId, $msgPerso, $msgPerso));
                }
            } else {
